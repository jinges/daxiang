...
$(function(){
	...
	$("#sum").keyup(function(){
		...
		if(params.buy == 'CNY') {
			...
			if(mo) {
				if(result){
					...
					yuan = duihuan(params.buy, currencyObj[params.sell][1], params.rate);
					sum.val(currencyObj[params.sell][1]);
					amount.val(yuan+"元人民币")
					alert('需输入最小兑换面额'+currencyObj[params.sell][1]+'的整数倍');
					return false;
				}
			...
			if(mo) {
				if(result) {
					...
					sum.val(currencyObj[params.buy][1]);
					yuan = duihuan(params.buy, currencyObj[params.buy][1], params.rate);
					amount.val(yuan+"元人民币")
					alert('需输入最小兑换面额'+currencyObj[params.buy][1]+'的整数倍');
					return false;
				}
		...
		if(yuan * 1 > params.max * 1) {
			amount.val(params.max+"元人民币");
			sum.val(Math.floor(params.max/params.rate));
			alert('单笔兑换金额不得高于'+ params.max +'元人民币');
			return false;
		}else if(yuan * 1 < params.min * 1) {
			amount.val(params.min+"元人民币");
			sum.val(Math.floor(params.min/params.rate));
